    ...
    const onSubmit = async () => {
      ...
      if (validation === '') {
        ...
        if (emailValid !== true && phoneValid !== true && newPasswordValid !== true && samePassword !== true) {
          ...
          try {
            const response = await ApiNewUser.get(`/user/otps?phone=${'91' + value?.Phone}`);
            ...
            Navigation.navigate({ 
              name: 'CitizensOtpVerifyScreen', 
              params: { phone: '91' + value?.Phone } 
            });
            console.log('Navigating to CitizensOtpVerifyScreen');
            ...
            console.log('API call error:', error); // Log the entire error object
    
            // If the error has a response property, log it separately
            if (error.response) {
          ...
          console.log('Validation failed: emailValid, phoneValid, newPasswordValid, or samePassword is false');
        }
      } else {
